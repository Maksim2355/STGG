...
Шаблонизаторы могут отличаться по формату входных данных для шаблонизации. Можно выделить следующие виды:

+ view-ориентированная структура. Подает на вход структурированные данные в виде языка разметки(XML) или текстового формата обмена данных(JSON, BEMJSON). Такой подход требует, чтобы входных данные были структурированны по определенным правилам, заложенным шаблонизатором. Приемущества такого подхода в том, что это облегчает работу программы, ей легче правильно создать шаблон и время на генерацию тратится меньше чем в других случаях.
+ Нормализованные данные. Используются "сырые" или сериализованные данные в одном из общепринятых форматах передачи данных, такие как JSON, yaml, xml и тд. Чаще всего такие данные являются результатами SQL запросов или запросов к API. В данном случае шаблонизатору приходится проделывать дополнительную работу для обработки данных и последующей шаблонизации, но все же данный подход имеет приемущество, ведь используются структурированные данные без приведении к специальной структуре, необходимый шаблонизации
+ Текст. Данный подход не имеет специальных требований для формата входных данных, в этом случае ответственность ложится на сам шаблонизатор. Ему необходимо эти данные прочитать, нормализировать и структурировать. Соответственно скорость шаблонизации снижается и шанс того, что выходные значения не будут соответствовать ожиданиям больше

## По парадигме

+ Императивные. В данном случае код будет выполнятся последовательно, обычно с помощью каких-то специальных вставок, предусмотренных программой. Примерами могут служить: препроцессоры, ERB, EJS и тд
+ Декларативные. Такой подход предусматировает описания преобразования с помощью какого-то DSL, который на вход принимает структурированные данные(чаще всего древовидные)
+ Смешанный тип. Используют оба подхода, основные преобразования описываются декларативным путем, а далее имеется возможность добавлять вставки на базовом языке

# Где используются шаблонизаторы

Шаблонизаторы отлично подходят, когда нам нужно

# Критерии для выбора шаблонизатора для выполнения конкретной задачи
